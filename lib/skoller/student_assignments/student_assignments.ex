defmodule Skoller.StudentAssignments do
  ...
  # good.
  def get_assignments_with_relative_weight(%{} = params) do
  ...
  def create_student_assignment(params) do
    ...
    changeset =
      Assignment.student_changeset(%Assignment{}, params)
    ...
    result =
      ...
      |> Ecto.Multi.run(:assignment, fn _, changes ->
        insert_or_get_assignment(changes, changeset)
      end)
      |> Ecto.Multi.run(:student_assignment, fn _, changes ->
        insert_student_assignment(changes, params)
      end)
      |> Ecto.Multi.run(:mod, fn _, changes ->
        Mods.insert_new_mod(changes, params["student_id"], params["is_private"])
      end)
      |> Repo.transaction()
      |> process_auto_update()
  ...
  def update_student_assignment(old, params) do
    ...
    result =
      ...
      |> Ecto.Multi.run(:mod, fn _, changes ->
        Mods.insert_update_mod(changes, changeset, params["is_private"])
      end)
      |> Repo.transaction()
      |> process_auto_update()
      |> mod_update_notification()
  ...
  def add_due_date_student_assignment(student_assignment, due) do
    student_assignment_query =
      where(StudentAssignment, [sa], sa.assignment_id == ^student_assignment.assignment_id)
    ...
    result =
      ...
      |> Ecto.Multi.run(:assignment, fn _, changes ->
        ...
        if due != nil and assignment.due == nil do
          ...
          {:error, "Assignment already has due date"}
        end
      end)
      |> Ecto.Multi.update_all(:update_student_assignments, student_assignment_query,
        set: [due: due]
      )
      |> Ecto.Multi.update(:update_class_assignment, fn %{assignment: assignment} ->
        Assignment.changeset(assignment, %{"due" => due})
      end)
      |> Ecto.Multi.run(:student_assignment, fn _, _ ->
        {:ok, Repo.get(StudentAssignment, student_assignment.id)}
      end)
      |> Repo.transaction()
  ...
  defp insert_or_get_assignment(_, %Ecto.Changeset{valid?: false} = changeset),
    do: {:error, changeset}

  defp insert_or_get_assignment(_, changeset) do
    assign =
      from(assign in Assignment)
  ...
  defp insert_student_assignment(%{assignment: %Assignment{} = assignment}, params) do
    ...
    student_assign =
      from(assign in StudentAssignment)
  ...
  defp validate_class_weight(%Ecto.Changeset{changes: %{weight_id: nil}} = changeset),
    do: changeset

  defp validate_class_weight(
         %Ecto.Changeset{changes: %{weight_id: weight_id}, valid?: true} = changeset
       ) do
  ...
  # good
  defp relative_weight_subquery(%StudentClass{id: id}) do
    query = from(assign in StudentAssignment)
  ...
  defp update_assignment(student_assignment) do
    case get_assignment_by_ids(
           student_assignment.assignment_id,
           student_assignment.student_class_id
         ) do
      nil ->
        :ok

      assign_old ->
        StudentAssignment.changeset_update_auto(assign_old, %{
