defmodule Skoller.Analytics.Classes do
  ...
  def get_community_classes() do
    ...
    |> join(:left, [p, ps, c, cs, s], sc_a in subquery(get_student_classes_active_subquery()), on: c.id == sc_a.class_id)
    |> join(:left, [p, ps, c, cs, s, sc_a], sc_i in subquery(get_student_classes_inactive_subquery()), on: c.id == sc_i.class_id)
