defmodule SkollerWeb.Api.V1.Organization.StudentOrgInvitationController do
  ...
  # plug :verify_owner, :student_org_invites when action in [:index, :get]

  def index(%{assigns: %{user: user}} = conn, %{"organization_id" => organization_id}) do
    case user do
      %{student: %{id: id}} ->
        invites = StudentOrgInvitations.get_by_params(student_id: id, organization_id: organization_id)
