defmodule ClassnavapiWeb.Helpers.ModHelper do
  ...
  def insert_new_mod(%{assignment: %Assignment{} = assignment}, params) do
    ...
    cond do
      ...
      true -> 
        ...
        |> Ecto.Multi.run(:auto_update, &Task.start(ModHelper, :process_auto_update, [existing_mod, &1]))
        |> Repo.transaction()
    end
  end
  ...
  def process_auto_update(mod, _) do
  ...
  defp insert_mod(mod, %StudentClass{} = student_class) do
    ...
    |> Ecto.Multi.run(:auto_update, &Task.start(ModHelper, :process_auto_update, [&1.mod, &1]))
  ...
  defp process_existing_mod(mod, %StudentClass{} = student_class, params) do
    case mod.is_private == true and is_private(params["is_private"]) == false do
      ...
      false -> 
        ...
        |> Ecto.Multi.run(:auto_update, &Task.start(ModHelper, :process_auto_update, [mod, &1]))
