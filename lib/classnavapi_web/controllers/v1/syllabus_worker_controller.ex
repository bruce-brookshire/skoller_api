defmodule ClassnavapiWeb.Api.V1.SyllabusWorkerController do
  ...
  defp get_ratios(_conn, status) do
    ...
    |> join(:inner, [class, period, sch], d in Doc, d.class_id == class.id)
    |> where([class], class.class_status_id == ^status and class.is_editable == true and class.is_syllabus == true)
    |> where([class, period, sch], sch.is_auto_syllabus == true)
    |> where([class, period, sch, d], d.is_syllabus = true)
  ...
  defp get_enrolled_ratios(_conn, status) do
    ...
    |> join(:inner, [class, period, sc, sch], d in Doc, d.class_id == class.id)
    |> where([class], class.class_status_id == ^status and class.is_editable == true and class.is_syllabus == true)
    |> where([class, period, sc, sch], sch.is_auto_syllabus == true)
    |> where([class, period, sc, sch, d], d.is_syllabus = true)
