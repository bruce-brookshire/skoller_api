defmodule ClassnavapiWeb.Jobs.SendNotifications do
  ...
  def run(time) do
    ...
    time
    |> future_assignment_query()
    |> Enum.each(&send_future_notifications(&1))
  end
  ...
  defp assignment_query(time) do
    ...
    |> where([student, sclass, sassign], fragment("?::date", sassign.due) == ^now)
  ...
  defp future_assignment_query(time) do
    {:ok, time} = Time.new(time.hour, time.minute, 0, 0)
    now = DateTime.utc_now() |> DateTime.to_date()
  ...
  defp send_future_notifications(assignment) do
    Notification.create_notification(assignment.udid, format_future_msg(assignment), @assignment_reminder_category)
  end
  ...
  defp format_future_msg(assignment) do
    assignment.assign_name <> " in " <> assignment.class_name <> " is due " <> due_days_away(assignment.assign_due) 
  end
