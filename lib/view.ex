  ...
  defmacro __using__(params) do
    ...
    quote do
      ...
      def render("show.json", %{unquote(single_atom) => model}) do
        ...
        associations = Model.__schema__(:associations) |> Enum.map(& {&1, render_association(Map.get(model, &1))})
      ...
      defp render_association(%{} = model), do: model |> Map.to_list() |> Enum.filter(&render_association/1) |> Map.new
      defp render_association({_name, %NotLoaded{}}), do: false
      defp render_association({:__meta__, _}), do: false
